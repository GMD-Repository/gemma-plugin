...
4. **Automated Layer Tree Organization**:
   ...
   - Applies matching QML styles from the plugin's bundled style library (`ref_mbi_cases.qml`, `ref_province_psa.qml`, `ref_province_lgu.qml`, `1. Base Layer Building Points.qml`). The `_boundary` table has no dedicated QML yet and loads with QGIS's default symbology.
