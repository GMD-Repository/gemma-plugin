...
2. **Digitize**:
   ...
   - **Adjust Feature**: Populates all barangay features from the selected **PSA Reference Layer** using its `barangay` attribute column. Features **Edit**, **Previous**, **Next**, and **Done** buttons. Clicking **Edit** selects the feature on the canvas, zooms to its bounding box, opens the Vertex Tool, activates the **Feature Edit Guard** (locking vertex edits strictly to the active feature ID and silently reverting accidental edits to neighboring polygons), auto-configures **Advanced Snapping** (12.0px tolerance, Vertex and Segment, Topological Editing, Self-Snapping enabled, Intersection Snapping disabled, Avoid Overlap), launches the core **Topology Checker** panel, minimizes the dialog, and displays a native dockable **Gemma Digitize Navigation Panel** (`DigitizeDockWidget`) defaulted to the lower-left area under the Layers Panel (`Previous`, `Next`, `Save & Done`, `Return`, and an embedded Feature Dropdown to jump directly to any feature). On every feature switch, **Validate All** is automatically triggered on the Topology Checker panel. Clicking **Save & Done** prompts the user to **Save**, **Discard**, or **Cancel** pending revisions.
     > 💡 **User Guideline**: Please ensure you configure your desired topology rules in the **Topology Checker** panel (e.g. `[PSA Reference Layer] -> must not have gaps`) once launched. The plugin will automatically trigger **Validate All** as you navigate through features.
