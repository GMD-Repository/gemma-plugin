...
| **Mismatch with Remarks** | Feature Sink (Polygon) | Cases marked `2_Pending` w/ 0 building points but with remarks present, or `1_Updated` w/ non-zero building points with remarks present. |
...
3. **Status Audit & Rule Evaluation**:
   - **Status Mismatch**: Cases reported as `1_Updated` (resolved) that are still physically detected, `2_Pending` cases with `0` building points and empty justification remarks, or `1_Updated` cases with remaining building points and empty remarks.
   - **Mismatch with Remarks**: Cases marked `2_Pending` with `0` building points but possessing written remarks, or `1_Updated` cases with remaining building points and non-empty justification remarks.
